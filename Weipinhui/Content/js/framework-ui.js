...
dialogOpen = function (options) {
    ...
    //_url = changeUrlParam(_url, "_", +new Date());
    var _width = top.$.windowWidth() > parseInt(options.width.replace('px', '')) ? options.width : top.$.windowWidth() + 'px';
    var _height = top.$.windowHeight() > parseInt(options.height.replace('px', '')) ? options.height : top.$.windowHeight() + 'px';
    var Content_IsClose = '<div style="float: left;padding-top:5px;padding-left:15px;-moz-user-select: none;-webkit-user-select: none;-ms-user-select: none;-khtml-user-select: none;user-select: none;"><input checked="checked" type="checkbox" id="IsdialogClose" style="vertical-align: middle;margin-bottom:2px;" /><label style=";padding-left:5px;cursor:pointer" for="IsdialogClose">确认并关闭窗口</label></div>';
    top.layer.open({
        id: options.id,
        type: 2,
        shade: options.shade,
        ...
        success: function (layero, index) {
            if (options.showClose && top.$("#IsdialogClose", layero).length == 0) {
                 $(".layui-layer-btn", layero).append(Content_IsClose);
            }
            Loading(false);
        },
        yes: function () {
...
dialogMsg = function (content, type) {
    ...
    top.layer.msg(content, { icon: type, time: 4000, shift: 5, zIndex: 1024 });
}
...
$.currentIframe = function () {
    if ($.isbrowsername() == "Chrome" || $.isbrowsername() == "Edge" || $.isbrowsername() == "FF") {
